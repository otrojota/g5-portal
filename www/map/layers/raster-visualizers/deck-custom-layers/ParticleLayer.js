...
import updateTransformVs from './shaders/particle-layer-update-transform.vs.glsl.js';
...
export default class ParticleLayer extends deck.LineLayer {
  getShaders() {
    return {
      ...super.getShaders(),
      inject: {
  ...
  _setupTransformFeedback() {
    ...
    const colors = device.createBuffer(new Float32Array(new Array(numInstances).fill(undefined).map((_, i) => {
    ...
    const transform = new deck.Transform(gl, {
      sourceBuffers: {
