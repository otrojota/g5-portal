import { ZVC, ZController } from "zvc";
...
class ZBootstrapTable extends ZController {
    ...
    async refresh(rowSelector) {
        ...
        if (this.pagination == "none" || this.pagination == "local") {
            ...
            if (rowSelector) {
                ...
                if (idx >= 0) await this.selectRowIndex(idx);
                else await this.selectRowIndex(-1);
            } else {
                await this.selectRowIndex(-1);
            }
        } else {
            this.selectedRowIndex = -1;
            this.createWorkingOverlay();
            try {
                this.totalRows = parseInt(await this.triggerEvent("getRowsCount"));
            } finally {
                this.removeWorkingOverlay();
            }
    ...
    recalcPagination() {
        this.totalPages = (this.totalRows / this.nRowsPerPage) || 1;
    ...
    async selectRowIndex(idx) {
        ...
        if (oldSelectedRowIndex != this.selectedRowIndex) {
            await this.triggerEvent("change", this.rows[this.selectedRowIndex], this.selectedRowIndex);
        }
    }
    getSelectedRow() {
